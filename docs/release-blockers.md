...
| Public website deployment | Blocked | Production website production URL `https://dropsquash.app/release-status` serves the release-status, privacy, pricing, license, support, and download pages on `dropsquash.app` | TBD | `https://...` |
...
| Benchmark release set | Verified | Release-set benchmark absolute CSV path outside repo is recorded and covers backend, saved percent, duration, speed ratio, short, medium, and large local samples, smaller outputs, machine/OS context, 20% regression threshold, and release candidate baseline | `docs/manual-qa.md` | `docs/manual-qa.md` |
...
|---|---|---|---|
| Packaged macOS manual QA | Manual packaged-app | Run the public `DropSquash.dmg` artifact matching the release notes Artifact URL through the manual QA table and record `manual-qa-check` passing | `docs/manual-qa.md` |
...
| Valid sandbox activation | License sandbox | Run the Lemon Squeezy sandbox activation request, confirm submit is disabled while Activating, and check local license cache 64-character lowercase hex fingerprint, `instance_id`, plus raw-key absence | `docs/manual-qa.md` |
...
| License network failure | License sandbox | Simulate a failed activation request and check the friendly error plus preserved local cache 64-character lowercase hex fingerprint, `instance_id`, and raw-key absence | `docs/manual-qa.md` |
| Expired license refresh | License sandbox | Attempt conversion with the expired offline grace cache, confirm the reconnect prompt, confirm conversion is blocked before starting, and check local license cache raw-key absence | `docs/manual-qa.md` |
| Local license forget | License sandbox | Use the local forget action, confirm the action is disabled while forgetting, confirm cache removal, and observe the trial or locked state | `docs/manual-qa.md` |
| Public website deployment | Public web | Deploy the production site and verify `https://dropsquash.app/release-status` serves release-status, privacy, pricing, license, support, and download pages | Public website URL |
| Pricing finalized | Public web | Publish the final pricing page URL at `https://dropsquash.app/pricing` and confirm no draft price copy remains before checkout goes live | Pricing URL |
| Refund policy finalized | Public web | Publish the final refund policy URL at `https://dropsquash.app/refund` and confirm it is linked before checkout goes live | Refund policy URL |
| Live checkout link | Public web | Verify the public pricing page opens the live `store.lemonsqueezy.com/checkout/buy/<id>` URL for the tested Lemon Squeezy checkout for the intended product | Live checkout URL |
| Signed DMG | Signing/notarization | Run `macos-signing-check`, sign the public `DropSquash.dmg` matching the release notes Artifact URL, and capture `codesign` Developer ID verification output | Release notes |
| Notarized and stapled DMG | Signing/notarization | Run `macos-signing-check`, notarize, confirm stapled status, and assess the public `DropSquash.dmg` matching the release notes Artifact URL with captured `spctl`, notary, and stapler verification output | Release notes |
...
| Benchmark release set | Benchmark | Run the release-set benchmark with short, medium, and large local recordings, record backend, saved percent, duration, speed ratio, the absolute CSV path outside repo, 20% threshold result, same-machine comparison, and release candidate baseline | `docs/manual-qa.md` |
...
| Homebrew cask install | Distribution | Open the Homebrew tap PR and verify `brew install --cask`, `brew uninstall --cask` removes it cleanly, versioned `DropSquash.dmg` Artifact URL, matching lowercase SHA-256, `auto_updates false`, and `zap` cleanup path | Homebrew tap PR URL |
...
| 3 | Public web proof | Public website deployment, Pricing finalized, Refund policy finalized, Live checkout link | Production `https://dropsquash.app/release-status`, `https://dropsquash.app/pricing`, and `https://dropsquash.app/refund` URLs serve release-status, privacy, pricing, license, support, download, checkout, and refund links before the public beta | Production website URLs |
