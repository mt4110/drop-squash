...
| 10 | Add license activation UI | In progress | Trial UI, locked activation form, local forget action, provider networking, local CLI status/forget, raw-key-free cache, empty-key and failed-activation cache safety, and deactivation-error redaction exist; sandbox/manual validation remains |
| 11 | Build release pipeline | In progress | Unsigned DMG QA artifact upload, release gates, artifact/checksum checks, release notes/refund URL gates, final publish gate, CI signing preflight, workflow coverage tests, and template coverage tests exist; signed/notarized publication remains |
