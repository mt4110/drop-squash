...
| 13 | Design DMG install cleanup | In progress | Finder copy or drag-to-Applications cannot run app code, so DropSquash must not promise automatic deletion of the downloaded `.dmg`; `load_install_location` can detect `/Volumes` launches and `/Applications` installs; the desktop UI warns when running from a disk image; a future explicit in-app install action may copy to `/Applications`, relaunch, eject the mounted volume, and only offer downloaded `.dmg` cleanup when the backing path is deterministically known; app move, volume eject, and user-triggered downloaded `.dmg` cleanup remain |
