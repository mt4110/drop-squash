...
| 5 License and trial UI | In progress | Convert trial usage into Pro unlock without account creation | License cache, instance id, activation/validation provider, local forget action, locked/Pro UI, local CLI status/forget | Raw license key is not persisted; empty/invalid/network/deactivation errors are friendly; valid cache survives grace period | Provider, trial UI, activation shell, local raw-key-free forget action, empty-key, network-failure, and failed-activation cache safety, local CLI status/forget, and safe cache/grace are implemented; Lemon Squeezy sandbox test remains |
| 6 Release pipeline | In progress | Ship a trusted macOS beta | Signed app, notarized DMG, checksums, release checklist, Homebrew cask draft | Gatekeeper opens without warning; secrets stay in CI; artifact checksum is published for the same DMG | Readiness, media/privacy security, read-only workflow permissions, unsigned DMG QA artifact upload, unsigned release failure gate, UDIF artifact, checksum, benchmark blocker, release notes URL/refund URL, same-DMG manual QA signing evidence, Gatekeeper no-warning evidence, numeric queue counts, verified smaller Trash output, Lemon Squeezy sandbox activation context, release notes prepared field-label synchronization, prepared draft placeholder rejection, conversion/queue/Trash/license action-state release notes evidence, final publish gate with release-notes URL reference matching, CI signing preflight, DMG bundle target, cask generation gates, and release-doc/workflow/template coverage tests exist; `docs/release-blockers.md` tracks external evidence; signing/notarization remain |
| 7 Sales site | In progress | Let a stranger understand, download, try, and buy | Landing, pricing, privacy, download, refund, FAQ, support | Privacy claims match implementation; CTA works; download path works | Static site draft, required-page gate, release-status/privacy/license/refund/support contact copy gates, pre-release CTA copy guard, download/checkout link and form action guard, actual website directory coverage, and live checkout/download link guard exist; hosting, final refund policy, and live checkout are tracked in `docs/release-blockers.md` |
...
| 8 | Add sequential queue | In progress | Queue model, React sequential runner, Rust queue commands for queued and active job states, web wire mapping, queued job cancellation, and batch summary are implemented; reproducible `docs/manual-qa.md` multi-file, queued-cancellation, and batch-summary evidence remains |
...
| 10 | Add license activation UI | In progress | Trial UI, locked activation form, local forget action, provider networking, local CLI status/forget, raw-key-free cache, empty-key, network-failure, and failed-activation cache safety, and deactivation-error redaction exist; sandbox/manual validation remains |
| 11 | Build release pipeline | In progress | Unsigned DMG QA artifact upload, read-only workflow permissions, unsigned release failure gate, release gates, artifact/checksum checks, same-DMG manual QA signing evidence, Gatekeeper no-warning evidence, release notes/refund URL gates, numeric queue counts, verified smaller Trash output, Lemon Squeezy sandbox activation context, release notes prepared field-label synchronization, prepared draft placeholder rejection, conversion/queue/Trash/license action-state release notes evidence, final publish gate with release-notes URL reference matching, CI signing preflight, workflow coverage tests, and template coverage tests exist; signed/notarized publication remains |
