...
| 5 License and trial UI | In progress | Convert trial usage into Pro unlock without account creation | License cache, instance id, activation/validation/deactivation, locked/Pro UI | Raw license key is not persisted; invalid/network errors are friendly; valid cache survives grace period | Trial UI, activation shell, and safe cache/grace are implemented; Lemon Squeezy provider remains |
...
| 10 | Add license activation UI | In progress | Trial UI, locked activation form, deactivate action, and raw-key-free cache exist; provider networking remains |
