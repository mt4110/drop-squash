...
| 13 | Design DMG install cleanup | Planned | Finder copy or drag-to-Applications cannot run app code, so DropSquash must not promise automatic deletion of the downloaded `.dmg`; a later first-launch helper may detect launch from `/Volumes`, offer to move the app to `/Applications`, eject the mounted installer volume after success, and leave deletion of the downloaded `.dmg` to the user |
