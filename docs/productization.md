...
| 9 | Wire source policy and Trash | In progress | Setting is persisted; Ask has explicit Trash action; Trash uses macOS NSFileManager after command-side output revalidation; `docs/manual-qa.md` Trash record remains |
| 10 | Add license activation UI | In progress | Trial UI, locked activation form, local forget action, provider networking, raw-key-free cache, and failed-activation cache safety exist; sandbox/manual validation remains |
