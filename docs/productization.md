...
| 3 Sequential queue | In progress | Handle multiple dropped files deterministically | Queue states, per-job progress, one active conversion at a time, queued job cancellation | Multiple drops create rows; one job runs at a time; queued cancellation prevents that job from starting; failures do not block unrelated jobs | Queue unit tests, `docs/qa-evidence.md`, plus reproducible `docs/manual-qa.md` multi-file and queued-cancellation evidence; `manual-qa-check` before completion |
...
| 8 | Add sequential queue | In progress | Queue model, React sequential runner, and queued job cancellation are implemented; reproducible `docs/manual-qa.md` multi-file and queued-cancellation evidence remains |
...
| 10 | Add license activation UI | In progress | Trial UI, locked activation form, local forget action, provider networking, local CLI status, raw-key-free cache, failed-activation cache safety, and deactivation-error redaction exist; sandbox/manual validation remains |
...
| 12 | Generate privacy receipt | Done | Successful CLI/desktop conversions write a local sidecar receipt with `uploaded_bytes = 0` and `metadata_policy = preserve`; desktop can reveal the saved receipt in Finder |
