...
- Benchmark regression threshold: mention the 20% regression threshold and whether any sample exceeded it.
...
- GitHub Release checksum: mention `SHA256SUMS` or `SHA-256`, `DropSquash.dmg`, that it is attached, the GitHub Release URL above, the Artifact URL above, and the exact SHA-256 digest above.
- Homebrew tap PR: mention the cask, PR, versioned `DropSquash.dmg`, the Artifact URL above, the exact SHA-256 digest above, `auto_updates false`, and `zap` cleanup path.
