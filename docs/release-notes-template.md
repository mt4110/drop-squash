...
- Benchmark sample set: mention short, medium, and large samples, smaller outputs, backend, CSV path outside the repository, machine, and OS context.
...
- Valid sandbox activation: mention the Lemon Squeezy sandbox request, disabled action state, Pro state, 64-character hex fingerprint, `instance_id`, `raw key`, and cache evidence.
