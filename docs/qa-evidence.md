...
| Release notes preparation | Release notes artifact fields, GitHub Release URL, SHA256SUMS line, pending checksum upload placeholder, and the matching Homebrew cask command are generated from the checked public `DropSquash.dmg`, version-matched public Artifact URL, checksum, version, and current git commit; stale artifacts older than `HEAD` are rejected | `cargo run -p xtask -- release-notes-prepare path/to/DropSquash.dmg <Artifact URL>` |
