...
| Manual QA completeness | Packaged-app evidence fields, concrete identity fields, absolute app-state paths, concrete results, malformed table rows, secret-like values, UDIF `.dmg` artifacts, same `.dmg` file evidence for artifact/checksum/signing rows, artifact-check/checksum path evidence, checksum digest matching the app artifact, same-machine release candidate benchmark baseline evidence, and Gatekeeper no-warning evidence are checked | `cargo run -p xtask -- manual-qa-check` |
