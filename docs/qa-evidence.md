...
| Release notes preparation | Release notes artifact fields and the matching Homebrew cask command are generated from the checked public `DropSquash.dmg`, version-matched public Artifact URL, checksum, version, and current git commit | `cargo run -p xtask -- release-notes-prepare path/to/DropSquash.dmg <Artifact URL>` |
