...
| Queue order | Sequential queue state transitions, QueueWorker events, active cancellation/failure continuation, queued cancellation, Rust/UI batch summary, and trial-lock pending-job blocking are tested | `cargo test -p dropsquash-queue && pnpm --dir apps/desktop/web lint` |
