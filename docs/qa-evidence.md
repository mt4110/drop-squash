...
| Homebrew cask generation | Cask generation rejects placeholders, non-semver versions, non-GitHub release URLs, imposter GitHub release hosts, nested artifact URLs, mismatched release versions, non-HTTPS URLs, non-DMG URLs, wrong DMG names, non-canonical homepages, invalid or placeholder checksums, declares no in-app auto-update, and includes the DropSquash app-state `zap` path | `cargo test -p xtask homebrew_cask` |
