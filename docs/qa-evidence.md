...
| Static site | Required pages, local links/resources, approved external links, placeholders, secret-like values, unsupported platform availability claims, release-status, privacy, license, refund, support contact copy, pre-release CTA copy, and pre-release download/checkout links or form actions are checked | `cargo run -p xtask -- website-check` |
...
| Homebrew cask generation | Cask generation rejects placeholders, non-semver versions, non-GitHub release URLs, nested artifact URLs, mismatched release versions, non-HTTPS URLs, non-DMG URLs, wrong DMG names, non-canonical homepages, invalid or placeholder checksums, declares no in-app auto-update, and includes the DropSquash app-state `zap` path | `cargo test -p xtask homebrew_cask` |
