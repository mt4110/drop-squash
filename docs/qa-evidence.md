...
| Benchmark harness | Local encoder benchmark argument parsing, release-set sample count, and output acceptance checks are tested | `cargo test -p xtask benchmark` |
...
| Release notes evidence | Filled public release notes reject placeholder URLs, checkout/website mix-up, missing signing/notary evidence, generic verification results, vague limitations, and vague support contact | `cargo run -p xtask -- release-notes-check path/to/release-notes.md` |
| Publish gate | Public beta publication requires release readiness, complete manual QA evidence, filled release notes evidence, and every release blocker marked Verified | `cargo run -p xtask -- publish-check path/to/release-notes.md` |
