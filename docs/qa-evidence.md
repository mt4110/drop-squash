...
| Release notes preparation | Release notes artifact fields, GitHub Release URL, SHA256SUMS line, post-upload checksum evidence text, and the matching Homebrew cask command are generated from the checked public `DropSquash.dmg`, version-matched public Artifact URL, checksum, version, and current git commit | `cargo run -p xtask -- release-notes-prepare path/to/DropSquash.dmg <Artifact URL>` |
