...
| Manual QA preparation | App state backup, repository-local app-state/state/output folder rejection, trial reset, required environment metadata, canonical artifact names, and `--restore-state` are tested without deleting config by default | `cargo test -p xtask manual_qa_prepare` |
