...
echo "next signing env: cd '$snapshot' && CARGO_TARGET_DIR=$target_dir cargo run -p xtask -- macos-signing-check"
echo "next distribution rerun: cd '$snapshot' && CARGO_TARGET_DIR=$target_dir cargo run -p xtask -- manual-qa-distribution-rerun '$prepared_md'"
