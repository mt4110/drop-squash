...
#[test]
fn publish_requires_release_notes_github_release_url() {
    let directory = tempfile::tempdir().unwrap();
    let path = directory.path().join("release-notes.md");
    std::fs::write(
        &path,
    ...
    assert!(error.contains("release notes must pass before publish"));
    assert!(error.contains("GitHub Release URL"));
}
...
#[test]
fn publish_requires_release_notes_public_website_url() {
    let directory = tempfile::tempdir().unwrap();
    let path = directory.path().join("release-notes.md");
    std::fs::write(
        &path,
...
#[test]
fn publish_requires_release_notes_pricing_url() {
    let directory = tempfile::tempdir().unwrap();
    let path = directory.path().join("release-notes.md");
    std::fs::write(
        &path,
...
#[test]
fn publish_requires_release_notes_refund_policy_url() {
    let directory = tempfile::tempdir().unwrap();
    let path = directory.path().join("release-notes.md");
    std::fs::write(
        &path,
