...
pub(super) fn ready_all_line(path: &Path, output_dir: &Path) -> String {
    line(
        "manual QA Ready all command",
        commands::ready_all_command(path, &output_dir.join("benchmark-results.csv")),
    )
}
...
pub(super) fn print_helper_commands(path: &Path, output_dir: &Path) {
    ...
    println!("{}", ready_all_line(path, output_dir));
