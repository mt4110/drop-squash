...
#[test]
fn reports_verified_network_failure_without_instance_id_evidence() {
    let blockers = "| License network failure | Verified | Friendly network error | `docs/manual-qa.md` | `docs/manual-qa.md` |\n";
...
fn accepts_verified_expired_refresh_with_reconnect_block_evidence() {
    ...
    let manual = "| Expired license refresh | Reconnect prompt | expired offline grace license.json cache showed reconnect prompt, blocked conversion before starting, and checked cache had no raw key |\n";

    assert!(missing_manual_verified_evidence(blockers, manual).is_empty());
}

#[test]
fn reports_verified_expired_refresh_without_cache_observation() {
    let blockers = "| Expired license refresh | Verified | Reconnect prompt before conversion | `docs/manual-qa.md` | `docs/manual-qa.md` |\n";
...
#[test]
fn reports_verified_local_forget_without_confirmed_removal() {
    let blockers = "| Local license forget | Verified | Local cache removed | `docs/manual-qa.md` | `docs/manual-qa.md` |\n";
    let manual = "| Forget license on this Mac | Local cache clears; app returns to trial or locked state | Forgetting state disabled action; license cache cleared and observed app returned to trial state |\n";
...
#[test]
fn reports_verified_local_forget_without_observed_state() {
    let blockers = "| Local license forget | Verified | Local cache removed | `docs/manual-qa.md` | `docs/manual-qa.md` |\n";
    let manual = "| Forget license on this Mac | Local cache clears; app returns to trial or locked state | Forgetting state disabled action; confirmed license cache cleared and app changed to trial state |\n";
...
#[test]
fn reports_verified_license_blocker_with_contradictory_raw_key_evidence() {
    let blockers = "| Valid sandbox activation | Verified | raw key is absent | `docs/manual-qa.md` | `docs/manual-qa.md` |\n";
    let manual = "| Valid sandbox activation | Pro state | Lemon Squeezy sandbox activation request entered Activating state, disabled submit, reached Pro state, and checked license.json cache kept fingerprint 1111111111111111111111111111111111111111111111111111111111111111 plus instance_id field with raw key absent but raw key persisted |\n";
...
#[test]
fn reports_verified_license_blocker_without_instance_id_evidence() {
    let blockers = "| Valid sandbox activation | Verified | raw key is absent | `docs/manual-qa.md` | `docs/manual-qa.md` |\n";
    let manual = "| Valid sandbox activation | Pro state; raw key absent from cache | Lemon Squeezy sandbox activation request entered Activating state, disabled submit, Pro reached; license.json cache checked fingerprint 1111111111111111111111111111111111111111111111111111111111111111 plus instance fields; raw key absent |\n";
...
#[test]
fn reports_verified_invalid_key_with_contradictory_raw_key_evidence() {
    let blockers = "| Invalid license key handling | Verified | raw key is absent | `docs/manual-qa.md` | `docs/manual-qa.md` |\n";
    let manual = "| Invalid key activation | Friendly license error | Activating state disabled submit; friendly error shown; license.json cache checked; raw key absent, no fingerprint, no instance, but raw key saved |\n";
...
#[test]
fn reports_packaged_macos_manual_qa_with_zero_blocked_lock_count() {
    let blockers = "| Packaged macOS manual QA | Verified | Filled manual QA table | `docs/manual-qa.md` | `docs/manual-qa.md` |\n";
    let manual = packaged_manual_qa_with(
        "Batch summary",
        "trial lock blocked pending jobs; summary showed finished count 2, saved bytes 123456, failed 0, cancelled 1, and blocked 0",
    );
...
#[test]
fn reports_packaged_macos_manual_qa_without_trash_only_after_evidence() {
    let blockers = "| Packaged macOS manual QA | Verified | Filled manual QA table | `docs/manual-qa.md` | `docs/manual-qa.md` |\n";
    let manual = packaged_manual_qa_with(
        "Trash source policy",
        "button showed Moving original and was disabled; original moved to Trash after verified smaller output",
    );
...
#[test]
fn reports_verified_benchmark_blocker_without_backend() {
    let blockers = "| Benchmark release set | Verified | Release-set benchmark CSV covers samples | `docs/manual-qa.md` | `docs/manual-qa.md` |\n";
    let manual = "\
...
fn accepts_verified_benchmark_blocker_with_percent_wording() {
    ...
    let manual = format!(
        "\
| `cargo run -p xtask -- benchmark --release-set --input <short> --input <medium> --input <large> --output-dir <tmp> --csv-output <tmp/results.csv>` | CSV recorded | benchmark CSV recorded for three samples with smaller outputs outside repo at csv={} |
| Benchmark sample set | Passes | short, medium, and large samples produced smaller outputs with backend apple-native on MacBookPro18,4 macOS 26.5.2 with CSV saved outside repo at csv:{} |
...
fn packaged_result(label: &str) -> String {
    match label {
        ...
        "Disk image launch notice" => {
            "app launched from mounted disk image under /Volumes, showed warning notice, Move copied DropSquash.app to Applications without replacing an existing app, Finder revealed the copied app, post-copy notice stayed visible, Open opened the installed app, and did not delete the downloaded .dmg".into()
        }
        "Choose recording conversion" => {
        ...
        "Batch summary" => {
            "trial lock blocked pending jobs; summary showed finished count 2, saved bytes 123456, failed 0, cancelled 1, and blocked 1".into()
        }
