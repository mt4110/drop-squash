...
fn packaged_result(label: &str) -> String {
    match label {
        ...
        "Disk image launch notice" => {
            "app launched from mounted disk image under /Volumes, showed warning notice, Move copied DropSquash.app to Applications without replacing an existing app, Finder revealed the copied app, post-copy notice stayed visible, Open opened the installed app, Quit closed the disk image copy, and did not delete the downloaded .dmg".into()
        }
        "Choose recording conversion" => {
