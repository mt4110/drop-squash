...
fn complete_manual_qa(artifact: &std::path::Path) -> String {
    ...
    for check in REQUIRED_CHECKS {
            ...
            == "`cargo run -p xtask -- benchmark --release-set --input <short> --input <medium> --input <large> --output-dir <tmp> --csv-output <tmp/results.csv>`"
        {
            ...
            text.push_str("| Disk image launch notice | Passes | app launched from mounted disk image under /Volumes, showed warning notice, Move copied DropSquash.app to Applications without replacing an existing app, Finder revealed the copied app, post-copy notice stayed visible, Open opened the installed app, and did not delete the downloaded .dmg |\n");
