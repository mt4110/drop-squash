...
- `spctl`: spctl --assess --type open accepted Developer ID source for public https://github.com/mt4110/drop-squash/releases/download/v0.1.0/DropSquash.dmg
...
- Apple notary log: notarytool accepted log request abc123 for public https://github.com/mt4110/drop-squash/releases/download/v0.1.0/DropSquash.dmg
...
#[test]
fn rejects_queue_evidence_without_waiting_row_context() {
    let errors = check_text(
        r#"
- Queue evidence: multi-file queue, queued cancellation, and batch summary showed trial lock blocked pending jobs with finished count 2, saved bytes 123456, failed 0, cancelled 1, and blocked 1
"#,
    );
...
#[test]
fn rejects_queue_evidence_with_zero_blocked_lock_count() {
    let errors = check_text(
        r#"
...
#[test]
fn rejects_spctl_and_notary_evidence_without_command_detail() {
    let errors = check_text(
        r#"
- Artifact URL: https://github.com/mt4110/drop-squash/releases/download/v0.1.0/DropSquash.dmg
...
#[test]
fn rejects_homebrew_install_without_artifact_url() {
    let errors = check_text(
        r#"
- Artifact URL: https://github.com/mt4110/drop-squash/releases/download/v0.1.0/DropSquash.dmg
- SHA-256: 0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef
- Homebrew install result: brew install --cask mt4110/tap/dropsquash completed for versioned DropSquash.dmg artifact with lowercase SHA-256 0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef and brew uninstall --cask mt4110/tap/dropsquash removed it cleanly
"#,
    );
...
#[test]
fn rejects_homebrew_install_without_uninstall_result() {
    let errors = check_text(
        r#"
- SHA-256: 0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef
...
#[test]
fn rejects_expired_refresh_without_conversion_attempt() {
    let errors = check_text(
        r#"
...
#[test]
fn rejects_checkout_url_outside_store_host() {
    let errors = check_text(
        r#"
- Live checkout URL: https://dropsquash.lemonsqueezy.com/checkout/buy/abc123
"#,
    );
...
#[test]
fn rejects_release_notes_manual_qa_record_without_artifact_url() {
    let errors = check_text(
        r#"
- Artifact URL: https://github.com/mt4110/drop-squash/releases/download/v0.1.0/DropSquash.dmg
...
"#,
    ...
    assert!(errors
        .iter()
        .any(|error| error.contains("Manual QA record must include the Artifact URL")));
}
