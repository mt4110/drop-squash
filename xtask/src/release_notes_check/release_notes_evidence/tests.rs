...
- Benchmark sample set: short medium large local recordings produced smaller outputs with backend apple-native on MacBookPro18,4 macOS 26.5.2 with CSV saved outside repo at /tmp/dropsquash-bench/results.csv
...
fn rejects_benchmark_sample_set_with_repo_local_csv_path() {
    ...
    let errors = check_text(&format!(
        "\
- Benchmark sample set: short medium large local recordings produced smaller outputs with backend apple-native on MacBookPro18,4 macOS 26.5.2 with CSV saved outside repo at {}
- Benchmark regression threshold: no sample exceeded 20% regression against the same-machine release candidate baseline
",
        csv.display()
    ));
