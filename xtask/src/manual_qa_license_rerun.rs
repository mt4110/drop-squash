...
const USAGE: &str = "usage: cargo run -p xtask -- manual-qa-license-rerun [manual-qa.md]";
...
pub(crate) fn run(args: Vec<String>) -> Result<(), String> {
    ...
    for line in status::lines(&path)? {
        println!("{line}");
    }
    crate::manual_qa_ready_license::run(vec![path.display().to_string()])
}
